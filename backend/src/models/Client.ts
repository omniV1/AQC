...
export interface IClientProfile {
  ...
  // Support Needs
  supportNeeds?: Array<
    'breastfeeding_support' | 'newborn_care' | 'emotional_support' | 'postpartum_recovery' |
    'sleep_guidance' | 'meal_preparation' | 'household_help' | 'partner_support' |
    'sibling_adjustment' | 'return_to_work'
  >;
...
// Virtual to calculate current postpartum week
...
clientSchema.statics.findNeedingAttention = function() {
  return this.find({
    $or: [
      { intakeCompleted: false, createdAt: { $lt: new Date(Date.now() - 3 * 24 * 60 * 60 * 1000) } }, // Incomplete intake after 3 days
