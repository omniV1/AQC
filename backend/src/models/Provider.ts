...
export interface IProfessionalInfo {
  certifications: Array<
    'DONA_Birth_Doula' | 'DONA_Postpartum_Doula' | 'CAPPA_Birth_Doula' | 'CAPPA_Postpartum_Doula' |
    'ToLabor_Birth_Doula' | 'ProDoula_Birth_Doula' | 'ProDoula_Postpartum_Doula' | 'Independent_Training' |
    'Lactation_Consultant_IBCLC' | 'Childbirth_Educator' | 'Prenatal_Yoga_Instructor' | 
    'Infant_Massage_Instructor' | 'Other'
  >;
  yearsExperience?: number;
  specialties: Array<
    'first_time_parents' | 'high_risk_pregnancies' | 'cesarean_recovery' | 'vbac_support' | 
    'multiples_twins' | 'breastfeeding_support' | 'postpartum_depression' | 'infant_sleep_guidance' |
    'newborn_care' | 'sibling_preparation' | 'loss_and_grief' | 'lgbtq_families' | 'teen_mothers' |
    'military_families'
  >;
  bio?: string;
  education: Array<{
...
export interface IProviderDocument extends Document {
  ...
  serviceAreas: string[];
  services: Array<
    'prenatal_support' | 'birth_support' | 'postpartum_support' | 'overnight_care' |
    'meal_preparation' | 'sibling_care' | 'household_assistance' | 'breastfeeding_support' |
    'sleep_training' | 'infant_care_education'
  >;
...
providerSchema.virtual('availabilityStatus').get(function(this: IProviderDocument): 'not_accepting' | 'full' | 'available' {
...
providerSchema.methods.addClient = function(
  ...
  // Check availability
  if (!this.availability.isAcceptingClients || 
...
providerSchema.methods.canAcceptNewClient = function(this: IProviderDocument): boolean {
  return this.availability.isAcceptingClients && 
         this.currentClientCount < this.availability.maxClients &&
         this.status === 'active';
};
