...
echo "Generating Andrew Mack Resource Library Project PDF..."
echo
...
pandoc "ANDREW_MACK_RESOURCE_LIBRARY_PROJECT.md" \
    --from markdown \
    --to pdf \
    ...
    --output="ANDREW_MACK_RESOURCE_LIBRARY_PROJECT.pdf" \
    --verbose

if [ $? -eq 0 ]; then
    echo
    echo "SUCCESS: PDF generated as 'ANDREW_MACK_RESOURCE_LIBRARY_PROJECT.pdf'"
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        # macOS
        open "ANDREW_MACK_RESOURCE_LIBRARY_PROJECT.pdf"
    elif [[ "$OSTYPE" == "linux-gnu"* ]]; then
        # Linux
        xdg-open "ANDREW_MACK_RESOURCE_LIBRARY_PROJECT.pdf" 2>/dev/null || echo "PDF generated. Please open manually."
    fi
else
    echo
