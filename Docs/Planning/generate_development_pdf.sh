...
echo "Generating LUNARA Development Phase PDF..."
echo
...
pandoc "Development_Phase_Coding_Testing.md" \
    --from markdown \
    --to pdf \
    ...
    --output="LUNARA_Development_Phase_Report.pdf" \
    --verbose

if [ $? -eq 0 ]; then
    echo
    echo "SUCCESS: PDF generated as 'LUNARA_Development_Phase_Report.pdf'"
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        # macOS
        open "LUNARA_Development_Phase_Report.pdf"
    elif [[ "$OSTYPE" == "linux-gnu"* ]]; then
        # Linux
        xdg-open "LUNARA_Development_Phase_Report.pdf" 2>/dev/null || echo "PDF generated. Please open manually."
    fi
else
    echo
